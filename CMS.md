...
但是我们不想让入口文件太过复杂，所以我们把它抽离到 global 文件夹中。
...
- 耦合度高，每个模块都直接依赖于第三方库（不在维护等问题）
- 每个请求都要携带token，或者有loading效果，或者拦截器，重复代码多

解决：

在项目与第三方库之间增加中间层进行封装

给 service 文件夹一个出口 index.ts 用于导出封装后的实例

构造实例，只需传入构造axios实例所需配置（如baseURL，拦截器，超时时间等）



拦截器分为全局（token，loading）和私有，想办法让拦截器具有高扩展性。传入config 创建axios 实例时可以传拦截器应用在实例上（私有的拦截器，创建实例时传入config，应用在实例上）单个请求拦截器在request方法里处理

拦截器粒度化处理，变得灵活，三层：所有实例  实例  单个请求，高扩展性
...
常见的有三种环境： 开发环境：development； 生产环境：production； 测试环境：test； 
...
有ref reactive useStore 生命周期钩子 就放到 hooks

没有的话就 config （配置文件） 比如表单验证规则
...
大组件拿到登录小组件的组件实例调用小组件的方法（登录逻辑），各自的数据以及逻辑只放在自己的小组件，防止大组件逻辑臃肿。

登录逻辑包括：

- 发请求，拿 token，存储 token
- 请求当前用户信息
- 鉴权拿到用户的菜单
- 跳转页面
...
记住密码模块时想到存到 localStorage 里，所以封装一个 localCache 模块（token 都能存储）



解决用户刷新或者直接拿 loaclStorage 登陆，vuex 里没有 token，用户信息的问题：

在项目入口处 setupStore：每次进入页面或者刷新时都会初始化，把本地的存储都放到 vuex 里



### 权限管理 （动态路由）

- 直接写死，全部注册。如果手动输入 url，就能进入页面（虽然页面里进行操作接口不给你反应），但是能进入页面就是不合理的，理应显示404
- 事先为不同的角色定好权限，登陆后拿到用户的角色，根据权限注册路由。弊端在于一旦角色或者权限变化，就得修改前端代码，重新部署
- 菜单动态动态生成路由映射。菜单里比如有用户管理， role 对应 url /main/role 对应路由 path，对应组件。一种情况是后端返回的菜单里就有 component 字段，这种就得要求组件名称、路径要和后端的完全统一。另一种后端只给菜单 url，前端配置好路径和组件的映射关系。



### 组件库二次封装（高阶组件）:

开发相似页面，做抽取。只需要写配置文件，组件中不需要写大量重复的代码，只需要传入配置 + 插槽即可。user -> search -> form 一层层封装。最终效果：整个CMS只需做一次封装，之后只写配置完成页面。

修改表单数据时，不想去修改props数据：解构重新定义响应式，watch 监听，触发自定义事件通知父组件

（视频8末尾）动态具名插槽+作用域插槽：user -> table ,封装 table 组件展示表单数据，想让展示的数据做一点处理，给父组件足够的控制权限。在page-content中动态插入剩余的插槽 把公共的和私有的分开



请求的逻辑封装到vuex里，请求xxlist



分了四层：页面层，组件层，baseUI层，vuex层管理数据。
...
setupStore注册动态路由要写在useVueRouter前面
